...
export const useCashManagement = (user) => {
  ...
  const carregarEstoque = useCallback(async () => {
    ...
    try {
      ...
      alert(
        'Atenção: Erro de conexão com o banco de dados. Detalhe: ' +
          error.message,
      )
    } finally {
      setIsLoading(false)
    ...
    try {
      ...
      Object.entries(estoqueLocal).forEach(([id, dados]) => {
        ...
        if (denom) {
          ...
          detalhamento_cofre[denom.valor.toString()] =
            Number(dados.unidades_atual) || 0
        }
      })

      // 3. Registra a conferência (Contagem Atual)
      ...
      // 4. SE houveram bolsas montadas na tela, registra a SAÍDA no histórico
      if (bolsasPendentes > 0) {
        const regraBolsa = [
          ...
          { valorFace: 2, qtd: 50 },
          { valorFace: 1, qtd: 5 },
          { valorFace: 0.5, qtd: 20 },
          { valorFace: 0.25, qtd: 40 },
          { valorFace: 0.1, qtd: 30 },
        ...
        await SupabaseCashRepository.registerMovement({
          ...
          valor_total: valorTotalBolsa,
          origem: 'Cofre Central',
          destino: 'Bolsa de Abertura (Caixa)',
          detalhamento: detalhamentoBolsa,
        })
      }

...
import { Calculator, Save, Loader2, Banknote, CreditCard, QrCode, ScrollText, Ticket, Calendar, Pencil, Trash2, XCircle, Send } from 'lucide-react'
...
const evaluateMath = (val) => {
  if (!val) return 0;
  try {
    let sanitized = String(val).replace(/,/g, '.').replace(/[^0-9.+\-]/g, '');
    if (!sanitized) return 0;
    
    // Remove operadores soltos no final da string para evitar erro
    sanitized = sanitized.replace(/[+\-]$/, '');
    
    // Captura os números junto com seus sinais (positivo ou negativo)
...
// Input customizado puro mantido: aceita strings complexas
...
export const PreClosing = () => {
  ...
  // === 2. VERIFICAÇÃO DE TURNO PARA O CAIXA TARDE ===
  ...
  const isCaixaTarde = role === 'CAIXA_TARDE';
  ...
  // SOLUÇÃO: Se for caixa tarde, a rua é cravada em 0
  const safeDelDinheiro = isCaixaTarde ? 0 : Number(deliveriesTotals?.dinheiro || 0);
  const safeDelCartao = isCaixaTarde ? 0 : Number(deliveriesTotals?.cartao || 0);
  const safeDelPix = isCaixaTarde ? 0 : Number(deliveriesTotals?.pix || 0);

  const dynDinheiro = calcCash + safeDelDinheiro
  const dynCartao = calcCard + safeDelCartao
  const dynPix = calcPix + safeDelPix
  const totalGeralProjetado = dynDinheiro + dynCartao + dynPix + calcCheck + calcVale

  // ================= FUNÇÕES DE SALVAR, EDITAR E WHATSAPP =================
  const handleSave = async () => {
    ...
    const payload = {
      ...
      pending_cash: safeDelDinheiro, pending_card: safeDelCartao, pending_pix: safeDelPix,
...
` : `
📈 *TOTAL GERAL:* R$ ${totalGeralProjetado.toFixed(2).replace('.', ',')}
`}
📝 *Observações:* ${formValues.obs_geral || 'Nenhuma'}`;
  ...
  const handleSendRowWhatsApp = (row) => {
    ...
    const temRua = !isCaixaTarde && ((pCash + pCard + pPix) > 0);
...
📝 *Observações:* ${row.obs_geral || 'Nenhuma'}`;
  ...
  const columns = [
    ...
    { header: 'TOTAL GERAL', render: (row) => <strong style={{ color: 'var(--color-primary)' }}>{formatReais(row.total)}</strong> },
    { 
      header: 'Ações', 
      render: (row) => (
        <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
          <button title="Enviar Relatório" onClick={() => handleSendRowWhatsApp(row)} style={{ background: 'none', border: 'none', color: '#16a34a', cursor: 'pointer' }}><Send size={18} /></button>
          {(user?.role === 'ADMIN' || user?.id === row.created_by) && (
            <>
              <button title="Editar" onClick={() => handleEditRow(row)} style={{ background: 'none', border: 'none', color: '#d97706', cursor: 'pointer' }}><Pencil size={18} /></button>
              <button title="Excluir" onClick={async () => { if(window.confirm('Excluir este fechamento?')) await deletePreClosing(row.id) }} style={{ background: 'none', border: 'none', color: '#dc2626', cursor: 'pointer' }}><Trash2 size={18} /></button>
            </>
          )}
        </div>
      )
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '16px' }}>
        ...
        <div style={{ display: 'flex', gap: '12px' }}>
          ...
          <Button variant="secondary" onClick={handleSendWhatsApp} icon={Send}>Enviar WhatsApp</Button>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '24px', alignItems: 'start' }}>
        ...
        <div style={{ backgroundColor: '#ffffff', borderRadius: '12px', border: '1px solid #e2e8f0', boxShadow: '0 4px 6px -1px rgba(0, 0, 0, 0.1)', padding: '24px', position: 'sticky', top: '24px' }}>
           ...
           {/* SOLUÇÃO: Os paineis da rua só aparecem se NÃO for o caixa da tarde */}
           {!isCaixaTarde && (
             <>
               <div style={{ borderTop: '1px dashed #cbd5e1', paddingTop: '16px', paddingBottom: '16px' }}>
                 ...
                 <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginTop: '12px' }}>
                   <div style={{ display: 'flex', justifyContent: 'space-between', color: '#b91c1c', fontSize: '0.9rem' }}>
                     <span>Dinheiro a receber:</span><strong style={{ color: '#dc2626' }}>+ R$ {safeDelDinheiro.toFixed(2).replace('.', ',')}</strong>
                   </div>
                   <div style={{ display: 'flex', justifyContent: 'space-between', color: '#b91c1c', fontSize: '0.9rem' }}>
                     <span>Cartão a receber:</span><strong style={{ color: '#ea580c' }}>+ R$ {safeDelCartao.toFixed(2).replace('.', ',')}</strong>
                   </div>
                   <div style={{ display: 'flex', justifyContent: 'space-between', color: '#b91c1c', fontSize: '0.9rem' }}>
                     <span>Pix a receber:</span><strong style={{ color: '#0ea5e9' }}>+ R$ {safeDelPix.toFixed(2).replace('.', ',')}</strong>
                   </div>
                 </div>
               </div>
           ...
           <div style={{ borderTop: isCaixaTarde ? '2px solid #e2e8f0' : '1px solid #bae6fd', paddingTop: '12px', marginTop: '12px' }}>
             {/* SOLUÇÃO: O Titulo muda se for Caixa Tarde */}
             <span style={{ fontSize: '0.85rem', fontWeight: 'bold', color: '#1e40af' }}>{isCaixaTarde ? 'TOTAL GERAL DA GAVETA' : 'TOTAL GERAL PROJETADO'}</span>
